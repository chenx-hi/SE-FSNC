...
from tree_utils import load_graph
from tree_utils import PartitionTree
...
def struct_tree(dataset, tree_depth=2):
    if not os.path.exists('trees'):
        os.makedirs('trees')
    if os.path.exists('trees/%s_%s.pickle' % (dataset, tree_depth)):
        return
    g_list = load_graph(dataset)
    pool_func = pool_trans_discon#pool_trans_discon #if dataset in discon_datasets else pool_trans
    pool = Pool()
...
def get_struct_tree(g_list, tree_depth=2):
    pool_func = pool_trans_discon #pool_trans_discon #if dataset in discon_datasets else pool_trans
    pool = Pool()
    g_list = pool.map(pool_func, [(g, tree_depth) for g in g_list])
    pool.close()
...
if __name__ == '__main__':
    datasets = ['ENZYMES', 'Letter_high', 'Reddit', 'TRIANGLES']
    for d in datasets:
